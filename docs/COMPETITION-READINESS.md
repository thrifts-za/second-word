...
| Distributed spend and abuse protection | Green | An atomic Durable Object enforces burst, daily, total, and 31 July deadline allowances before Gloo runs. It stores counters only, never drafts or identities. Invalid rewrite tokens are rejected before allowance is consumed; `test/gloo-budget.test.ts`, `test/provider-budget.test.ts`, and `test/request-boundary.test.ts`. |
...
| --- | --- | --- |
| **Live Gloo AI Studio** | **Green: deployed and verified** | On 2026-07-22, `REQUIRE_GLOO=1 npm run preflight` passed the deployed Guard, impact breadth, Guide, Silence, signed Rewrite, tamper rejection, YouVersion upstream, and Verse of the Day paths on Gemini 2.5 Flash Lite. Encrypted Cloudflare secrets hold the credentials. An atomic Durable Object allowance caps Gloo at 300 analyses and 75 rewrites daily, 2,500 and 600 total through 31 July; one budgeted retry absorbs transient failures, and Workers AI remains the truthful availability fallback. |
| YouVersion Highlights data exchange | Out of scope | User-authorized Highlights require a separate YouVersion account and callback flow. The competition demo does not need it, and no mocked sync is claimed. Presence already provides the valuable, verified daily-word experience without account access. |
| One real extension surface | Yellow | Gmail compose and received-message selectors were verified live on 2026-07-21. Reload the latest `extension/dist` and repeat one end-to-end reply before recording, noting Chrome version and date. The automated MV3 and Gmail gates remain green. |
| Public repo contains current code | Green | `origin/main` matches the local submission repository and is publicly reachable at `https://github.com/thrifts-za/second-word`. |
| Valid Kaggle submission | Red: external publication remains | Private Kaggle notebook run, <=500-word writeup, cover image, accessible <=3-minute YouTube video, public project link, logged-out link check, and final Kaggle submission. The notebook may remain private until competition visibility changes after the deadline. |
...
6. Show `/health` and the deployed Gloo preflight, then return to the human
   outcome rather than adding an unbuilt account-sync feature.
